...
def display_box(image, box):
    x, y, w, h = box
    cv2.rectangle(image, (x, y), (x+w, y+h), (100, 10, 55))


def check_if_target_stable(object_center, stable_check_box):
    ...
    try:
        while running:
            ...
            if not key_up:
                ...
                elif very_last_pressed_key == 0 or very_last_pressed_key == 82:  # Key Up
                    drone.move_up()
                elif very_last_pressed_key == 1 or very_last_pressed_key == 84:  # Key Down
                    drone.move_down()
            ...
            if object_center:
                ...
                if abs(object_center_delta_height_rel) > 0.2:
                    speed_y = object_center_delta_height_rel  # up , down
            ...
            if should_hold_altitude and key_up:
                ...
                stable_check_box = (center_x, center_y, 50, 50)
                display_box(drone_camera_image, stable_check_box)

                if check_if_target_stable(object_center, stable_check_box):
                    drone
                else:
                    drone.apply_velocity_down_cam(speed_x, speed_y, speed_z)
...
def point_is_in_box((px,py), (bx, by, bw, bh))
    in_x_range = px <= bx + bw and px >= bx
    in_y_range = py <= by + bh and py >= by
